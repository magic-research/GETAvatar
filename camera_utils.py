...
class LookAtPoseSampler:
    ...
    def sample(horizontal_mean, vertical_mean, lookat_position, horizontal_stddev=0, vertical_stddev=0, radius=1, batch_size=1, device='cpu'):
        ...
        camera_origins[:, 0:1] = radius*torch.sin(phi) * torch.cos(theta)
        camera_origins[:, 2:3] = radius*torch.sin(phi) * torch.sin(theta)
        camera_origins[:, 1:2] = radius*torch.cos(phi) + lookat_position[1]
...
def create_cam2world_matrix(forward_vector, origin):
    ...
    left_vector = math_utils.normalize_vecs(torch.cross(up_vector, forward_vector, dim=-1))

    up_vector = math_utils.normalize_vecs(torch.cross(forward_vector, left_vector, dim=-1))
