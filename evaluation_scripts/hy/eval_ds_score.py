...
def generate_ds_images(G, psi=1, truncation_cutoff=14, cfg='FFHQ',device=torch.device('cuda'), label_pool='', outdir='', **video_kwargs):
    ...
    img_outdir = os.path.join(outdir, 'exp_shape_var/shape_var')
    if not os.path.exists(img_outdir):
        os.mkdir(img_outdir)

    # exp_seed = np.random.randint(0, len(frame_3dmms))+1
    exp_seed = 2
    ...
    zs =  torch.from_numpy(np.stack([np.random.RandomState(seed).randn(G.z_dim) for seed in [47]])).to(device)
    ws = G.mapping(zs, conditioning_params, truncation_psi=psi, truncation_cutoff=truncation_cutoff)
    camera_params = torch.from_numpy(frame_3dmms[exp_seed:exp_seed+1, :25]).to(device).float()
    ...
    for frame_idx in tqdm(range(num_images)):
        ...
        for seed in [another_seed]:
            frame_3dmm = frame_3dmms[seed:seed+1, -feature_dim:]
            if frame_idx == 7:
                exp = frame_3dmm[..., 100:]
            ...
            # frame_3dmm[..., 100:] = torch.from_numpy(frame_3dmm[..., 125:]).to(device).float()
            # frame_3dmm[..., :100] = torch.randn_like(frame_3dmm[..., 25:125])
            rendering_params = torch.cat([conditioning_params[...,:25].reshape(-1, 25), frame_3dmm.reshape(-1, feature_dim)], 1)

            out = G.synthesis(ws, rendering_params.repeat([len(ws), 1]), ws_bcg=ws_bcg, noise_mode='const')
