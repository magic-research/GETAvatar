...
class DMTetGeometry(Geometry):
    def __init__(
            self, grid_res=64, scale=2.0, cano_bbox=None,
            device='cuda', renderer=None,
        ...
        if cano_bbox is None:
            ...
            assert scale == 2.0
            self.verts = self.verts * scale
            self.cano_verts = cano_bbox[0:1, :] + (self.verts + 1.0) * (cano_bbox[1:2, :] - cano_bbox[0:1, :]) * 0.5
    ...
    def render_mesh(self, mesh_v_nx3, mesh_f_fx3, camera_mv_bx4x4, resolution=256, hierarchical_mask=False, camera_type='blender'):
        return_value = dict()
