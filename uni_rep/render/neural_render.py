...
class NeuralRender(Renderer):
    def __init__(self, device='cuda', blender_camera_model=None, smpl_camera_model=None):
        super(NeuralRender, self).__init__()
        self.device = device
        self.ctx = None
        self.projection_mtx = None
        self.blender_camera = blender_camera_model
        self.smpl_camera = smpl_camera_model
    ...
    def render_mesh(
        ...
        if camera_type=='blender':
            v_pos_clip = self.blender_camera.project(v_pos)  # Projection in the camera
        elif camera_type=='smpl':
            v_pos_clip = self.smpl_camera.project(v_pos)  # Projection in the camera
        else:
