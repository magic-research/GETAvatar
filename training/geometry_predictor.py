...
from training.smpl_utils import grid_sample
...
class TriPlaneTexGeo(torch.nn.Module):
    def __init__(
        ...
        if cano_bbox is not None:
            self.cano_bbox = cano_bbox.reshape(1, 2, 3)
        else:
            self.cano_bbox = None
        ...
        self.mlp_synthesis_def = ImplicitSynthesisNetwork(
            ...
            input_channel=mlp_input_channel,
            device=device)
        self.mlp_synthesis_norm = ImplicitSynthesisNetwork(
            out_channels=3,
            n_layers=self.n_implicit_layer,
            w_dim=self.w_dim,
            latent_channel=mlp_latent_channel,
    ...
    def get_normal_prediction(self, sdf_feature, position, ws_geo):
        '''
        ...
        if self.cano_bbox is not None:
            normalized_tex_pos = 2.0 * (position - self.cano_bbox[:, 0:1, :]) / (self.cano_bbox[:, 1:2, :] - self.cano_bbox[:, 0:1, :]) - 1.0
        ...
        final_feat = (x_feat + y_feat + z_feat)
        final_feat = final_feat.squeeze(dim=2).permute(0, 2, 1)  # 32dimension
        norm = self.mlp_synthesis_norm(ws_geo, final_feat)

        return norm

    def get_sdf_def_prediction(self, sdf_feature, position, ws_geo, gradient_detach=True):
        '''
        Predicting SDF and deformation for the vertices
        :param sdf_feature: triplane feature for geometry
        :param position: location for the tetrahedral grid vertices
...
class SuperresolutionHybrid4X(torch.nn.Module):
    def __init__(self, 
                channels, img_resolution, sr_num_fp16_res, 
                sr_antialias, num_fp16_res=4, conv_clamp=None, 
                channel_base=None, channel_max=None,# IGNORE
                device='cuda',
                **block_kwargs):
        super().__init__()
        ...
        self.block0 = SynthesisBlockNoUp(
                channels, 128, w_dim=512, resolution=img_resolution,
                img_channels=3, is_last=False, use_fp16=use_fp16, 
                conv_clamp=(256 if use_fp16 else None),
                device=device,
                **block_kwargs)

        self.block1 = SynthesisBlockNoUp(
                128, 64, w_dim=512, resolution=img_resolution,
                img_channels=3, is_last=True, 
                use_fp16=use_fp16, conv_clamp=(256 if use_fp16 else None), 
                device=device,
                **block_kwargs)
...
class OSGDecoder(torch.nn.Module):
    def __init__(self, n_features, device):
        super().__init__()
        self.hidden_dim = 64
        self.decoder_output_dims = 32
        self.decoder_lr_mul = 1.0
        # FullyConnectedLayer(w_dim, in_channels, bias_init=1, device=device)
        self.net = torch.nn.Sequential(
            FullyConnectedLayer(n_features, self.hidden_dim, lr_multiplier=self.decoder_lr_mul, device=device),
            torch.nn.Softplus(),
            FullyConnectedLayer(self.hidden_dim, self.decoder_output_dims, lr_multiplier=self.decoder_lr_mul, device=device)
        )
      
    def forward(self, sampled_features):
        # Aggregate features
        # sampled_features = sampled_features.mean(1) 
        # x = sampled_features.reshape(sampled_features[0], -1, sampled_features[-1])
        res = sampled_features.shape[1]
        x = sampled_features.view(sampled_features.shape[0], res * res, sampled_features.shape[-1])
        ...
        rgb = rgb.view(N, res, res, self.decoder_output_dims)
        return rgb
