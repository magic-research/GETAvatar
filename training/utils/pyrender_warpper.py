...
def render_color_and_normal_and_depth_buffers(mesh, camera, camera_transform, light=None, light_transform=None, resolution=512):
    global suppress_multisampling
    suppress_multisampling = True

    # Important
    scene = pyrender.Scene(ambient_light=[1.2,1.2,1.2]) # bg_color=[1.,1.,1.,0.]
    scene.add(pyrender.Mesh.from_trimesh(mesh, smooth=True))

    scene.add(camera, pose=camera_transform)
    if light != None: 
        scene.add(light, pose=light_transform)
    ...
    normal, _ = renderer.render(scene, flags=pyrender.RenderFlags.SKIP_CULL_FACES)
...
def render_color_and_depth_buffers(mesh, camera, camera_transform, light=None, light_transform=None, resolution=512):
    global suppress_multisampling
    suppress_multisampling = True

    # Important
    # scene = pyrender.Scene.from_trimesh_scene(mesh,ambient_light=[0.4,0.4,0.4],bg_color=[0.,0.,0.,0.])
    scene = pyrender.Scene(ambient_light=[1.2,1.2,1.2], bg_color=[0.,0.,0.,0.]) # bg_color=[1.,1.,1.,0.]
    # mtl = pyrender.Material(emissiveFactor=[0.1,0.1,0.1]) # emissiveFactor=[0.,0.,0.]
    # mtl = pyrender.MetallicRoughnessMaterial()
    # mtl = pyrender.material.SpecularGlossinessMaterial(specularFactor=[0.,0.,0.])
    # scene.add(pyrender.Mesh.from_trimesh(mesh, material=mtl, smooth=True))
    scene.add(pyrender.Mesh.from_trimesh(mesh, smooth=True))
