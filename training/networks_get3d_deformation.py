...
from training.geometry_predictor import MappingNetwork, ToRGBLayer, \
    TriPlaneTexGeo, SuperresolutionHybrid4X, SuperresolutionHybrid8X, FullyConnectedLayer
...
class DMTETSynthesisNetwork(torch.nn.Module):
    def __init__(
        ...
        obs_bbox_y_center = torch.tensor([0.0, 0.5 * (self.obs_bbox_y_max + self.obs_bbox_y_min), 0.0]).to(self.device).float()
        ...
        if self.use_deformation:
            self.offset_network = OffsetNetwork(depth=4, skips=[3], width=128, freqs_xyz=10, scale=0.1, scale_type='clamp')
        else:
        ...
        self.register_buffer('obs_bbox_y_center', obs_bbox_y_center)
    ...
    def canonical_mapping(self, coords, verts, body_bbox, ober2cano_transform, ws_offset, geo_branch=False):
        ...
        if self.use_deformation:
            assert self.offset_network is not None
            try:
                if geo_branch:
                    filtered_coords_offset = self.deformation(self.offset_network, filtered_coords_cano.detach(), filtered_coords.detach(), ws_offset)
                else:
                    filtered_coords_offset = self.deformation(self.offset_network, filtered_coords_cano.detach(), filtered_coords.detach(), ws_offset)
                filtered_coords_offset = torch.nan_to_num(filtered_coords_offset, 0.0)
                filtered_coords_offset = torch.clamp(filtered_coords_offset, -1e2, 1e2)
                filtered_coords = filtered_coords_cano + filtered_coords_offset
                # replace with original pts if deformed out of canonical bbox
                boxed_coords_deformed = 2 * (filtered_coords-self.canonical_bbox[:,0:1]) / (self.canonical_bbox[:,1:2]-self.canonical_bbox[:,0:1]) - 1
                deform_mask = boxed_coords_deformed.abs().amax(-1) >= 1
                filtered_coords[deform_mask] = filtered_coords_cano[deform_mask]
                offset_reg = filtered_coords_offset[filtered_coords_mask].norm(dim=-1).mean()
            except:
        ...
        return filtered_coords, filtered_coords_ori, mask, canonical_filtered_coords_mask, max_length, offset_reg

    def canonical_mapping_normal(self, coords, verts, body_bbox, ober2cano_transform, ws_offset, geo_branch=False):
        mask, filtered_coords_mask, max_length, filtered_coords, _ = self.filter_and_pad(coords, body_bbox)
        #filtered_coords_ori = filtered_coords.clone()
        ...
        return filtered_coords, filtered_rotate_matrix, mask, canonical_filtered_coords_mask, max_length, offset_reg
    
    def deformation(self, offset_network, coords_cano, coords, ws=None):
        input_coords = torch.cat([coords_cano, coords],dim=-1)
        deformed_coords = offset_network(input_coords, ws)
    ...
    def get_sdf_deformation_prediction(
            self, ws, ws_offset=None, position=None, sdf_feature=None,
        ...
        if self.one_3d_generator:
            init_position = init_position + self.obs_bbox_y_center.detach()[None, None]
            # cano_position, batch_obs_body_sdf, batch_mask = self.inverse_skinning(init_position, **canonical_mapping_kwargs)
            sample_coordinates_cano, sample_coordinates_obs, mask, coordinates_mask, max_length, offset_reg = self.canonical_mapping(init_position, ws_offset=ws_offset, geo_branch=True, **canonical_mapping_kwargs)
            ...
            sample_coordinates_cano = sample_coordinates_cano - self.obs_bbox_y_center.detach()[None, None]
            sdf_cano, deformation_cano = self.generator.get_sdf_def_prediction(
                sdf_feature, ws_geo=ws,
                position=sample_coordinates_cano)
            
            sdf_cano = sdf_cano + batch_cano_body_sdf
        ...
        if torch.sum(zero_surface).item() > 0:
            ...
            max_sdf = sdf.max()
            min_sdf = sdf.min()
            update_sdf[:, self.dmtet_geometry.center_indices] = update_sdf[:, self.dmtet_geometry.center_indices] + (1.0 - min_sdf)  # greater than zero
            update_sdf[:, self.dmtet_geometry.boundary_indices] = update_sdf[:, self.dmtet_geometry.boundary_indices] + (-1 - max_sdf)  # smaller than zero
            new_sdf = torch.zeros_like(sdf)
            for i_batch in range(zero_surface.shape[0]):
                if zero_surface[i_batch]:
                    new_sdf[i_batch:i_batch + 1] = new_sdf[i_batch:i_batch + 1] + update_sdf
        ...
        return sdf, deformation, sdf_reg_loss, offset_reg

    def get_geometry_prediction(self, ws, sdf_feature=None, canonical_mapping_kwargs=None, ws_offset=None):
        '''
        ...
        sdf, deformation, sdf_reg_loss, offset_reg = self.get_sdf_deformation_prediction(
            ws, ws_offset,
        ...
        for i_batch in range(n_batch):
            ...
            verts = verts + self.obs_bbox_y_center.detach()[None]
            v_list.append(verts)
            f_list.append(faces)

        return v_list, f_list, sdf, deformation, v_deformed, sdf_reg_loss, offset_reg


    def get_texture_prediction(self, ws, tex_pos, hard_mask, tex_feature, canonical_mapping_kwargs=None, ws_offset=None):
        '''
        ...
        tex_coordinates_cano, tex_coordinates_obs, tex_mask, tex_coordinates_mask, tex_max_length, offset_reg = self.canonical_mapping(tex_pos, ws_offset=ws_offset, **canonical_mapping_kwargs)

        tex_coordinates_cano = tex_coordinates_cano - self.obs_bbox_y_center.detach()[None, None]
        ...
        return final_tex_feat.reshape(ws.shape[0], hard_mask.shape[1], hard_mask.shape[2], final_tex_feat.shape[-1]), offset_reg

    def get_texture_prediction_with_eikonal(self, ws, tex_pos, ws_geo, hard_mask, tex_feature, sdf_feature, canonical_mapping_kwargs=None, ws_offset=None):
        '''
        Predict Texture given latent codes
        :param ws: Latent code for texture generation
        :param tex_pos: Position we want to query the texture field
        ...
        return final_tex_feat.reshape(ws.shape[0], hard_mask.shape[1], hard_mask.shape[2], final_tex_feat.shape[-1]), eikonal_loss, offset_reg

    def get_normal_prediction(self, surface_pos, ws_geo, hard_mask, sdf_feature, canonical_mapping_kwargs=None, return_eikonal=False, ws_offset=None):
        '''
        Predict Texture given latent codes
        ...
        surface_coordinates_cano, surface_coordinates_rotate_matrix, surface_mask, surface_coordinates_mask, surface_max_length, offset_reg = self.canonical_mapping_normal(surface_pos, ws_offset=ws_offset, **canonical_mapping_kwargs)

        surface_coordinates_cano = surface_coordinates_cano - self.obs_bbox_y_center.detach()[None, None]
        ...
        return final_surface_normal_obs.reshape(ws_geo.shape[0], hard_mask.shape[1], hard_mask.shape[2], 3), eikonal_loss, offset_reg
    ...
    def extract_3d_shape(
        ...
        if self.one_3d_generator:
            ...
            # mesh_v, mesh_f, sdf, deformation, v_deformed, sdf_reg_loss, offset_reg = self.get_geometry_prediction(ws_geo, sdf_feature)
            mesh_v, mesh_f, _, _, _, _, _ = self.get_geometry_prediction(ws_geo, sdf_feature)
        else:
    ...
    def generate_normal_map(
        ...
        if self.one_3d_generator:
            ...
            ws_offset = torch.cat((ws_tex[:, -1], ws_geo[:, -1]), dim=-1).detach()
            mesh_v, mesh_f, sdf, deformation, v_deformed, sdf_reg_loss, offset_reg_geo = self.get_geometry_prediction(ws_geo, sdf_feature, canonical_mapping_kwargs, ws_offset=ws_offset)
        else:
            raise NotImplementedError
            #mesh_v, mesh_f, sdf, deformation, v_deformed, sdf_reg_loss = self.get_geometry_prediction(ws_geo)
        ...
        if self.one_3d_generator:
            normal_obs_feat, eikonal_loss, offset_reg_norm = self.get_normal_prediction(
                surface_pos, ws_geo, surface_hard_mask,
                sdf_feature, canonical_mapping_kwargs=canonical_mapping_kwargs,
                return_eikonal=return_eikonal, ws_offset=ws_offset)
        else:
            raise NotImplementedError
        ...
        return final_img, antilias_mask, sdf, deformation, v_deformed, mesh_v, mesh_f, img_buffers_viz, \
               mask_pyramid, surface_hard_mask, sdf_reg_loss, eikonal_loss, return_value, offset_reg_geo+offset_reg_norm
    ...
    def generate(
        ...
        if self.one_3d_generator:
            if return_eikonal:
                ...
                tex_feat, eikonal_loss, offset_reg_tex = self.get_texture_prediction_with_eikonal(
                    ws_tex, tex_pos, ws_geo, tex_hard_mask,
                    tex_feature, sdf_feature, 
                    canonical_mapping_kwargs=canonical_mapping_kwargs, 
                    ws_offset=ws_offset)
            else:
                eikonal_loss = None
                # get_texture_prediction(ws, tex_pos, hard_mask, tex_feature, canonical_mapping_kwargs)
                tex_feat, offset_reg_tex = self.get_texture_prediction(
        ...
        return final_img, antilias_mask, sdf, deformation, v_deformed, mesh_v, mesh_f, img_buffers_viz, \
               mask_pyramid, tex_hard_mask, sdf_reg_loss, eikonal_loss, return_value, offset_reg_geo+offset_reg_tex
    ...
    def forward_normal(self, ws, c, ws_geo, return_shape=False, return_eikonal=False, **block_kwargs):
        ...
        tex_hard_mask, sdf_reg_loss, eikonal_loss, render_return_value, offset_reg_geo = self.generate_normal_map(ws, c, ws_geo, return_eikonal=return_eikonal, **block_kwargs)
        if return_shape:
            return normal_img, sdf, deformation, v_deformed, mesh_v, mesh_f, mask_pyramid, sdf_reg_loss, eikonal_loss, render_return_value, offset_reg_geo
        return normal_img, mask_pyramid, sdf_reg_loss, eikonal_loss, render_return_value, offset_reg_geo
    ...
    def forward(self, ws, c, ws_geo, return_shape=False, return_eikonal=False, **block_kwargs):
        ...
        tex_hard_mask, sdf_reg_loss, eikonal_loss, render_return_value, offset_reg_geo = self.generate(ws, c, ws_geo, return_eikonal=return_eikonal, **block_kwargs)
        if return_shape:
            return img, sdf, deformation, v_deformed, mesh_v, mesh_f, mask_pyramid, sdf_reg_loss, eikonal_loss, render_return_value, offset_reg_geo
        return img, mask_pyramid, sdf_reg_loss, eikonal_loss, render_return_value, offset_reg_geo
...
class GeneratorDMTETMesh(torch.nn.Module):
    ...
    def generate_3d(
        ...
        sdf_reg_loss, eikonal_loss, render_return_value, _ = self.synthesis.generate(
            ws, c=c,
            ws_geo=ws_geo,
            **synthesis_kwargs)
        ...
        normal_img, _, _, _, _, _, _, _, _, _, _, _, _, _ = self.synthesis.generate_normal_map(
            ws, c, 
            ws_geo=ws_geo,
            **synthesis_kwargs)
        ##############################
    ...
    def forward(
        ...
        img, _, _, _, _, _, _, _, _, _, _ = self.synthesis(
            ws=ws, 
            c=c,
            ws_geo=ws_geo,
            update_emas=update_emas,
...
class OffsetNetwork(torch.nn.Module):
    ...
    def forward(self, xyz, ws_offset=None):
        xyz_encoded = self.encoding_xyz(xyz)
        h = torch.cat([xyz_encoded, ws_offset[:, None].repeat(1, xyz_encoded.size(1), 1)], -1)
        B, N, C = h.shape
        h = h.reshape(B*N, C)
...
class Embedder(torch.nn.Module):
    def __init__(self, in_channels=3, N_freqs=5, logscale=True):
        """
