...
def setup_snapshot_image_grid(training_set, random_seed=0, inference=False, load_normal_map=True):
    ...
    # Load data.
    images, labels, masks, normal_images = zip(*[training_set[i][:4] for i in grid_indices])

    if load_normal_map:
        return (gw, gh), np.stack(images), np.stack(labels), masks, np.stack(normal_images)
    else:
        return (gw, gh), np.stack(images), np.stack(labels), masks, None
...
def training_loop(
        ...
        G_norm_interval=0,  # How often to perform norm intervel
        D_norm_interval=0,  # How often to perform norm intervel
    ...
    grid_sample_gradfix.enabled = True  # Avoids errors with the augmentation pipe. TODO: set to false
    ...
    if dis_pose_cond:
        ...
        common_kwargs = dict(
            c_dim=0, img_resolution=training_set.resolution, img_channels=training_set.num_channels)
    G_kwargs['device'] = device
    D_kwargs['device'] = device

    D_kwargs['normal_dis_pose_cond'] = normal_dis_pose_cond
    ...
    for name, module, opt_kwargs, reg_interval, norm_interval in [('G', G, G_opt_kwargs, G_reg_interval, G_norm_interval),
                                                   ('D', D, D_opt_kwargs, D_reg_interval, D_norm_interval)]:
        if reg_interval is None:
        ...
        elif G_norm_interval==0:  # Lazy regularization.
            ...
            opt = dnnlib.util.construct_class_by_name(
                ...
                **opt_kwargs)  # subclass of torch.optim.Optimizer
            phases += [dnnlib.EasyDict(name=name + 'main', module=module, opt=opt, interval=1)]
            phases += [dnnlib.EasyDict(name=name + 'norm', module=module, opt=opt, interval=norm_interval)]
            if name=='D':
                phases += [dnnlib.EasyDict(name=name + 'reg', module=module, opt=opt, interval=reg_interval)]
                phases += [dnnlib.EasyDict(name=name + 'reg_norm', module=module, opt=opt, interval=reg_interval)]
    ...
    if rank == 0:
        ...
        if load_normal_map:
            grid_size, images, labels, masks, normal_images = setup_snapshot_image_grid(training_set=training_set, inference=inference_vis, load_normal_map=load_normal_map)
        else:
            grid_size, images, labels, masks, _ = setup_snapshot_image_grid(training_set=training_set, inference=inference_vis, load_normal_map=load_normal_map)
        masks = np.stack(masks)
        masks = np.clip(masks + 127.5, a_min=0, a_max=255)
        if load_normal_map:
            images = np.concatenate((images, masks[:, np.newaxis, :, :].repeat(3, axis=1), normal_images), axis=-1)
        else:
            images = np.concatenate((images, masks[:, np.newaxis, :, :].repeat(3, axis=1)), axis=-1)
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            phase_real_img, phase_real_c, real_mask, phase_real_normal_img = next(training_set_iterator)
            # phase_real_img torch.Size([4, 3, 1024, 1024])
            # phase_real_c.shape  torch.Size([4, 2])
            # real_mask.shape  torch.Size([4, 1024, 1024])
            ...
            # combine normal map
            if load_normal_map:
                phase_real_normal_img = (phase_real_normal_img.to(device).to(torch.float32) / 127.5 - 1)
                phase_real_normal_img = torch.cat([phase_real_normal_img, real_mask], dim=1)
                phase_real_normal_img = phase_real_normal_img.split(batch_gpu)        
            #
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            # NOTE: different
            phase.opt.zero_grad(set_to_none=False) # why not set to none? set to zero instead?
            phase.module.requires_grad_(True)

            if load_normal_map:
                for real_img, real_normal_img, real_c, gen_z, gen_c in zip(phase_real_img, phase_real_normal_img, phase_real_c, phase_gen_z, phase_gen_c):
                    loss.accumulate_gradients(
                        phase=phase.name, real_img=real_img, real_norm_img=real_normal_img, real_c=real_c, gen_z=gen_z, gen_c=gen_c,
                        gain=phase.interval, cur_nimg=cur_nimg)
            else:
                for real_img, real_c, gen_z, gen_c in zip(phase_real_img, phase_real_c, phase_gen_z, phase_gen_c):
                    loss.accumulate_gradients(
                        phase=phase.name, real_img=real_img, real_norm_img=None, real_c=real_c, gen_z=gen_z, gen_c=gen_c,
                        gain=phase.interval, cur_nimg=cur_nimg)
        ...
        if (network_snapshot_ticks is not None) and (done or cur_tick % network_snapshot_ticks == 0) and not detect_anomaly:  # and (cur_tick != 0 or resume_pretrain is not None):  ###########
            ...
            for key, value in snapshot_data.items():
                # may change  dr.ops.RasterizeGLContext to dr.ops.RasterizeCudaContext
