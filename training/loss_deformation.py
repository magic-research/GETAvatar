...
class StyleGAN2Loss(Loss):
    ...
    def run_G(
        ...
        if return_shape:
            img, sdf, deformation, v_deformed, mesh_v, mesh_f, mask_pyramid, sdf_reg_loss, eikonal_loss, render_return_value, offset_reg_geo = self.G.synthesis(
                ws, c,
                ws_geo=ws_geo,
                return_shape=return_shape,
                return_eikonal=return_eikonal
            )
            return img, sdf, ws, deformation, v_deformed, mesh_v, mesh_f, mask_pyramid, ws_geo, sdf_reg_loss, eikonal_loss, render_return_value, offset_reg_geo
        else:
            img, mask_pyramid, sdf_reg_loss, eikonal_loss, render_return_value, offset_reg_geo = self.G.synthesis(
                ws, c, 
                ws_geo=ws_geo,
                return_shape=return_shape,
                return_eikonal=return_eikonal
        ...
        return img, ws, mask_pyramid, render_return_value, offset_reg_geo
    ...
    def run_G_normal(
        ...
        # Step 3: Generate rendered image of 3D generated shapes.
        if return_shape:
            normal_img, sdf, deformation, v_deformed, mesh_v, mesh_f, mask_pyramid, sdf_reg_loss, eikonal_loss, render_return_value, offset_reg_geo = self.G.synthesis.forward_normal(
                ws, c,
                ws_geo=ws_geo,
                return_shape=return_shape,
                return_eikonal=return_eikonal
            )
            return normal_img, sdf, ws, deformation, v_deformed, mesh_v, mesh_f, mask_pyramid, ws_geo, sdf_reg_loss, eikonal_loss, render_return_value, offset_reg_geo
        else:
            normal_img, mask_pyramid, sdf_reg_loss, eikonal_loss, render_return_value, offset_reg_geo = self.G.synthesis.forward_normal(
                ws, c, 
                ws_geo=ws_geo,
                return_shape=return_shape,
                return_eikonal=return_eikonal)
        ...
        return normal_img, ws, mask_pyramid, render_return_value, offset_reg_geo
    ...
    def run_D_normal(self, img, c, update_emas=False, mask_pyramid=None):
    ...
    def accumulate_gradients(
        ...
        if phase in ['Gmain']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                sdf_reg_loss, eikonal_loss, render_return_value, offset_reg_geo = self.run_G(
                    gen_z, gen_c, return_shape=True
                )

                gen_logits = self.run_D(gen_img, gen_c, mask_pyramid=mask_pyramid)
                ...
                # Regularization loss for offset prediction
                if offset_reg_geo is not None:
                    loss_Goffsetreg_norm = (offset_reg_geo).abs().mean().mul(1)
                    training_stats.report('Loss/G/loss_offsetreg', loss_Goffsetreg_norm)
                    loss_Gmain += loss_Goffsetreg_norm
        ...
        if phase in ['Gnorm']:
            with torch.autograd.profiler.record_function('Gnorm_forward'):
                ...
                sdf_reg_loss, eikonal_loss, render_return_value, offset_reg_geo = self.run_G_normal(
                    gen_z, gen_c, return_shape=True, return_eikonal=True
                )

                gen_logits = self.run_D_normal(gen_normal_img, gen_c, mask_pyramid=mask_pyramid)
        ...
        if phase in ['Dmain']:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                # First generate the rendered image of generated 3D shapes
                gen_img, _gen_ws, mask_pyramid, render_return_value, _ = self.run_G(
                    gen_z, gen_c, update_emas=True)

                gen_logits = self.run_D(
                    gen_img, gen_c, update_emas=True, mask_pyramid=mask_pyramid)
        ...
        if phase in ['Dnorm']:
            with torch.autograd.profiler.record_function('Dgen_norm_forward'):
                ...
                gen_normal_img, _gen_ws, mask_pyramid, render_return_value, _ = self.run_G_normal(
                    gen_z, gen_c, update_emas=True)
                self.G.requires_grad_(False)

                gen_logits = self.run_D_normal(
                    gen_normal_img.detach(), gen_c, update_emas=True, mask_pyramid=mask_pyramid)
        ...
        if phase in ['Dnorm', 'Dreg_norm']:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                real_logits = self.run_D_normal(real_norm_img_tmp, real_c, )
