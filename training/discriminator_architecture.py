...
class Discriminator(torch.nn.Module):
    def __init__(
            ...
            img_channels,  # Number of input color channels.
            normal_dis_pose_cond, # use pose conditioning for normal discriminator or not
            architecture='resnet',  # Architecture: 'orig', 'skip', 'resnet'.
        ...
        super().__init__()
        self.normal_dis_pose_cond = normal_dis_pose_cond
        # self.conditional_dim = c_dim
        self.c_dim = c_dim
        self.c_cam_dim = 16 # add the world2cam matrix here
        self.img_resolution = img_resolution
        self.img_resolution_log2 = int(np.log2(img_resolution))

        self.img_channels_drgb = img_channels
        self.img_channels_dmask = 1
        self.img_channels_dnorm = 3
        ...
        if self.c_dim > 0:
            self.mapping = MappingNetwork(z_dim=0, c_dim=self.c_cam_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, device=device, **mapping_kwargs)
        ...
        if self.c_dim > 0:
            self.mask_mapping = MappingNetwork(
                z_dim=0, c_dim=self.c_cam_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, device=device,
                **mapping_kwargs)
        self.mask_b4 = DiscriminatorEpilogue(
            mask_channels_dict[4], cmap_dim=cmap_dim, resolution=4, device=device, **epilogue_kwargs,
            **common_kwargs)

        # Step 3: set up discriminator for normal image
        common_kwargs = dict(img_channels=self.img_channels_dnorm, architecture=architecture, conv_clamp=conv_clamp)
        cur_layer_idx = 0
        for res in self.block_resolutions:
            in_channels = channels_dict[res] if res < img_resolution else 0
            tmp_channels = channels_dict[res]
            ...
            block = DiscriminatorBlock(
                ...
                first_layer_idx=cur_layer_idx, use_fp16=use_fp16, device=device, **block_kwargs, 
                **common_kwargs)
            setattr(self, f'norm_b{res}', block)
            cur_layer_idx += block.num_layers
        if self.normal_dis_pose_cond and self.c_dim > 0:
            self.norm_mapping = MappingNetwork(
                z_dim=0, c_dim=self.c_cam_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, device=device, **mapping_kwargs)
            self.norm_b4 = DiscriminatorEpilogue(
                channels_dict[4], cmap_dim=cmap_dim, resolution=4, device=device, **epilogue_kwargs, 
                **common_kwargs)
        else:
            self.norm_b4 = DiscriminatorEpilogue(
                channels_dict[4], cmap_dim=0, resolution=4, device=device, **epilogue_kwargs, 
                **common_kwargs)

    def forward_normal(self, img, c, update_emas=False, alpha=1.0, mask_pyramid=None, **block_kwargs):
        # Step 1: feed the mask image into the discriminator
        _ = update_emas
        mask_x = None
        img_res = img.shape[-1]
        mask_img = img[:, self.img_channels_dnorm: self.img_channels_dnorm + self.img_channels_dmask]  # This is only supervising the geometry
        for res in self.block_resolutions:
        ...
        if self.c_dim > 0:
            ...
            c_cam = c[:,:self.c_cam_dim]
            mask_cmap = self.mask_mapping(None, c_cam)
        mask_x = self.mask_b4(mask_x, mask_img, mask_cmap)

        # Step 2: feed the normal image into another discriminator
        img_for_normal = img[:, :self.img_channels_dnorm, :, :]
        _ = update_emas
        normal_x = None
        img_res = img_for_normal.shape[-1]
        for res in self.block_resolutions:
            block = getattr(self, f'norm_b{res}')
            normal_x, img_for_normal = block(normal_x, img_for_normal, alpha, (img_res // 2) == res, **block_kwargs)
        normal_cmap = None
        if self.normal_dis_pose_cond and self.c_dim > 0:
            c_cam = c[:,:self.c_cam_dim]
            normal_cmap = self.norm_mapping(None, c_cam)
        normal_x = self.norm_b4(normal_x, img_for_normal, normal_cmap)
        return normal_x, mask_x
...
class MultiDiscriminator(torch.nn.Module):
    def __init__(
        ...
        # part disc
        self.p_img_resolution = img_resolution//4
        ...
        # Step 1: set up discriminator for RGB image
        common_kwargs = dict(img_channels=self.img_channels_drgb, architecture=architecture, conv_clamp=conv_clamp)
        common_kwargs_p = dict(img_channels=self.img_channels_drgb, architecture=architecture, conv_clamp=conv_clamp)
        cur_layer_idx = 0
        for res in self.block_resolutions:
            in_channels = channels_dict[res] if res < img_resolution else 0
            tmp_channels = channels_dict[res]
        ...
        # Step 2: set up discriminator for mask image
        common_kwargs = dict(img_channels=self.img_channels_dmask, architecture=architecture, conv_clamp=conv_clamp)
        common_kwargs_p = dict(img_channels=self.img_channels_dmask, architecture=architecture, conv_clamp=conv_clamp)
        cur_layer_idx = 0
        mask_channel_base = channel_base
        mask_channel_max = channel_max
        mask_channels_dict = {res: min(mask_channel_base // res, mask_channel_max) for res in self.block_resolutions + [4]}
        ...
        # Step 3: set up discriminator for normal image
        common_kwargs = dict(img_channels=self.img_channels_dnorm, architecture=architecture, conv_clamp=conv_clamp)
        common_kwargs_p = dict(img_channels=self.img_channels_dnorm, architecture=architecture, conv_clamp=conv_clamp)
        cur_layer_idx = 0
        for res in self.block_resolutions:
            in_channels = channels_dict[res] if res < img_resolution else 0
            tmp_channels = channels_dict[res]
    ...
    def forward_normal_bk(self, img, c, part_img, update_emas=False, alpha=1.0, mask_pyramid=None, **block_kwargs):
        # Step 0: resize part img to the same resolution
        part_img = filtered_resizing(part_img, size=img.shape[-1]//4, f=self.resample_filter)
        ...
        img_res = img.shape[-1]
        mask_img = img[:, self.img_channels_dnorm: self.img_channels_dnorm + self.img_channels_dmask]  # This is only supervising the geometry
        mask_img_part = part_img[:, self.img_channels_dnorm: self.img_channels_dnorm + self.img_channels_dmask]  # This is only supervising the geometry
        for res in self.block_resolutions:
            block = getattr(self, f'mask_b{res}')
            mask_x, mask_img = block(mask_x, mask_img, alpha, (img_res // 2) == res, **block_kwargs)
            if res in [self.p_img_resolution, self.p_img_resolution//2, self.p_img_resolution//4]:
                p_block = getattr(self, f'mask_pb{res}')
                mask_x_part, mask_img_part = p_block(mask_x_part, mask_img_part, **block_kwargs)
            if res in [self.p_img_resolution//4]:
                mask_x = torch.cat([mask_x, mask_x_part], 1)
        mask_cmap = None
        if self.c_dim > 0:
            # mask_cmap = self.mask_mapping(None, c)
            c_cam = c[:,:self.c_cam_dim]
        ...
        # Step 2: feed the normal image into another discriminator
        img_for_normal = img[:, :self.img_channels_dnorm, :, :]
        img_for_normal_part = part_img[:, :self.img_channels_dnorm, :, :]
        _ = update_emas
        normal_x = None
        normal_x_part = None
        img_res = img_for_normal.shape[-1]
        for res in self.block_resolutions:
            ...
            normal_x, img_for_normal = block(normal_x, img_for_normal, alpha, (img_res // 2) == res, **block_kwargs)
            if res in [self.p_img_resolution, self.p_img_resolution//2, self.p_img_resolution//4]:
                p_block = getattr(self, f'norm_pb{res}')
                normal_x_part, img_for_normal_part = p_block(normal_x_part, img_for_normal_part, **block_kwargs)
            if res in [self.p_img_resolution//4]:
                normal_x = torch.cat([normal_x, normal_x_part], 1)
        normal_cmap = None
        if self.normal_dis_pose_cond and self.c_dim > 0:
            c_cam = c[:,:self.c_cam_dim]
            normal_cmap = self.norm_mapping(None, c_cam)
    ...
    def forward_normal(self, img, c, part_img, update_emas=False, alpha=1.0, mask_pyramid=None, **block_kwargs):
        # Step 1: feed the mask image into the discriminator
        _ = update_emas
        mask_x = None
        img_res = img.shape[-1]
    ...
    def forward_bk(self, img, c, part_img, update_emas=False, alpha=1.0, mask_pyramid=None, **block_kwargs):
        # Step 0: resize part img to the same resolution
        part_img = filtered_resizing(part_img, size=img.shape[-1]//4, f=self.resample_filter)
        ...
        img_res = img.shape[-1]
        mask_img = img[:, self.img_channels_drgb:self.img_channels_drgb + self.img_channels_dmask]  # This is only supervising the geometry
        mask_img_part = part_img[:, self.img_channels_drgb: self.img_channels_drgb + self.img_channels_dmask]  # This is only supervising the geometry
        for res in self.block_resolutions:
            block = getattr(self, f'mask_b{res}')
            mask_x, mask_img = block(mask_x, mask_img, alpha, (img_res // 2) == res, **block_kwargs)
            if res in [self.p_img_resolution, self.p_img_resolution//2, self.p_img_resolution//4]:
        ...
        # Step 2: feed the RGB image into another discriminator
        img_for_tex = img[:, :self.img_channels_drgb, :, :]
        img_for_tex_part = part_img[:, :self.img_channels_drgb, :, :]
        _ = update_emas
        x = None
        x_part = None
        img_res = img_for_tex.shape[-1]
        for res in self.block_resolutions:
            ...
            x, img_for_tex = block(x, img_for_tex, alpha, (img_res // 2) == res, **block_kwargs)
            if res in [self.p_img_resolution, self.p_img_resolution//2, self.p_img_resolution//4]:
                p_block = getattr(self, f'pb{res}')
                x_part, img_for_tex_part = p_block(x_part, img_for_tex_part, **block_kwargs)
            if res in [self.p_img_resolution//4]:
    ...
    def forward(self, img, c, part_img, update_emas=False, alpha=1.0, mask_pyramid=None, **block_kwargs):
        # Step 0: resize part img to the same resolution
        part_img = filtered_resizing(part_img, size=img.shape[-1]//4, f=self.resample_filter)
...
def filtered_resizing(image_orig_tensor, size, f, filter_mode='antialiased'):
    if filter_mode == 'antialiased':
        ada_filtered_64 = torch.nn.functional.interpolate(image_orig_tensor, size=(size, size), mode='bilinear', align_corners=False) #, antialias=True) -- Not available in PyTorch 1.10.0
        ...
        filtered = torch.nn.functional.interpolate(image_orig_tensor, size=(size, size), mode='bilinear', align_corners=False) #, antialias=True) -- Not available in PyTorch 1.10.0
        aliased  = torch.nn.functional.interpolate(image_orig_tensor, size=(size, size), mode='bilinear', align_corners=False) #, antialias=False) -- Not available in PyTorch 1.10.0
